 ...
 *
 */
public class CustomAutoComplete extends CustomAutoCompleteComboBox 
{
	 ...
	 *  @private
	 */
	protected var tempCollection:Object;
	...
	[Bindable] public var initialLoad:Boolean = false;
	...
	override protected function keyDownHandler(event:KeyboardEvent):void
	{
	    ...
	    if(!event.ctrlKey)
		{
		    ...
		    if(event.keyCode == Keyboard.UP && prevIndex==0)
			{
			    ...
			    TextInput(textInput).setSelection(textInput.text.length, textInput.text.length);
			    selectedIndex = -1; 
			}
						      ...
						      unscaledHeight:Number):void
	{
	    ...
	    if(dropdown)
		{
		    if(typedTextChanged)
			{
			    ...
			    dropdown.dataTipFunction = getToolTip;
  			    if(lookAhead && showDropdown && typedText!="" && !removeHighlight)
				{
					...
					if(index==0)
					{
					    ...
					    TextInput(textInput).setSelection(textInput.text.length,_typedText.length);
					}
					else
					{
					    textInput.text = _typedText;
		    	...
		    	//arrows keys.
				TextInput(textInput).setSelection(_typedText.length,textInput.text.length);
		}
