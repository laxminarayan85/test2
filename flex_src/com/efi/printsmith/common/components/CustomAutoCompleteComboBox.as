 ...
 *
 */
public class CustomAutoCompleteComboBox extends ComboBase
                      ...
                      IListItemRenderer
{
	...
	[Bindable] public var parentCloseEventTriggered:Boolean = false;
	[Bindable] public var selectedIndexValue:int=0;
	[Bindable] public var changeItemRenderer:Boolean = false;
     ...
     *  Constructor.
     */
    public function CustomAutoCompleteComboBox()
    {
        super();
        ...
        // CustomAutoCompleteComboBox to be lost.
     ...
     *
     *  <p>The CustomAutoCompleteComboBox control uses the <code>listData</code> property and the
     *  <code>data</code> property as follows. If the CustomAutoCompleteComboBox is in a 
     *  DataGrid control, it expects the <code>dataField</code> property of the 
     *  column to map to a property in the data and sets 
     *  <code>selectedItem</code> to that property. If the CustomAutoCompleteComboBox control is 
     ...
     *  CustomAutoCompleteComboBox to select that item (display it in the TextInput portion and set
     *  the selectedIndex) if it exists in the dataProvider.
     *  <p>If the CustomAutoCompleteComboBox control is editable, the <code>selectedItem</code>
    ...
    /**
     *  A reference to the List control that acts as the drop-down in the CustomAutoCompleteComboBox.
     *
    ...
    /**
     *  The prompt for the CustomAutoCompleteComboBox control. A prompt is
     *  a String that is displayed in the TextInput portion of the
     *  CustomAutoCompleteComboBox when <code>selectedIndex</code> = -1.  It is usually
     *  a String like "Select one...".  If there is no
     *  prompt, the CustomAutoCompleteComboBox control sets <code>selectedIndex</code> to 0
    ...
    /**
     *  Maximum number of rows visible in the CustomAutoCompleteComboBox control list.
     *  If there are fewer items in the
     *  dataProvider, the CustomAutoCompleteComboBox shows only as many items as
    ...
    /**
     *  The String displayed in the TextInput portion of the CustomAutoCompleteComboBox. It
    ...
    {
        if (CustomAutoCompleteComboBox.createAccessibilityImplementation != null)
            CustomAutoCompleteComboBox.createAccessibilityImplementation(this);
    }
    ...
    /**
     *  @private
     *  Make sure the drop-down width is the same as the rest of the CustomAutoCompleteComboBox
     */
    ...
    public function close(trigger:Event = null):void
    {
        if (_showingDropdown)
        {
            if (_dropdown) {
            	trace("close");
				parentCloseEventTriggered = true;
            	selectedIndexValue = _dropdown.selectedIndex;
    ...
    private function displayDropdown(show:Boolean, trigger:Event = null):void
    {
        ...
        var point:Point = new Point(dropDownX, unscaledHeight);
    ...
    override protected function collectionChangeHandler(event:Event):void
    {
        ...
        if (event is CollectionEvent)
        {
            ...
            if (collection.length == 0)
            {
                ...
                // if the CustomAutoCompleteComboBox is non-editable remove the text
