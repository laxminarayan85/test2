...
public class ImportServlet extends HttpServlet implements Servlet {
	...
	void importData(File uploadedFile, String importType, XpdexImportParams importParams) throws Exception {
		...
		if (importType != null && "ChargeDefinitions".equals(importType)) {
			...
			mapper.importFile(uploadedFile);
		} else if (importType != null && "TableEditor".equals(importType)) {
			mapper = new TableEditorMapper();
			mapper.importFile(uploadedFile);
		} else {
			if (importType != null) {
				...
				} else if ("CreditCardTransactions".equals(importType)) {
					mapper = new CreditCardTransactionsMapper();
				} else if ("Address".equals(importType)) {
					mapper = new AddressMapper();
				} else if ("Employee".equals(importType)) {
