...
// Put here to keep from bloating LAProxy
...
public class Base64
{
  ...
  /** The carriage return character (\r) as a byte. */
  private final static byte CARRIAGE_RETURN = (byte)'\r';
  ...
  **/
  ...
  public static void main(String[] args) throws Exception {
	if ( args.length < 1 || args.length > 3 ) {
		System.err.println("Usage: Base64 (encode|decode) [(inURL|inFilePath) [(outURL|outFilePath)]]");
		System.err.println("\noutURL defaults to stdout if none is provided");
		System.err.println("inURL defaults to stdin if none is provided");
		return;
	}

	String action = args[0];
	if ( ! ("decode".equalsIgnoreCase(action) || "encode".equalsIgnoreCase(action)) ) {
	...
	if ( args.length < 2 ) {
		...
		is = (inURL==null) ? new FileInputStream(new File(input)) : inURL.openStream();
	}
	...
	if ( args.length < 3 ) {
		...
		URL outURL = null;
		try {
			outURL = new URL(output);
		} catch (MalformedURLException ex) {
		}
		os = (outURL==null) ? new FileOutputStream(new File(output)) : outURL.openConnection().getOutputStream();
	}

	if ( "decode".equalsIgnoreCase(action) ) {
		java.io.InputStream result = Base64.decode(is);
		int read = 0;
		...
		os.write(Base64.encode(is).getBytes());
	}
...
/* ********  E N C O D I N G   M E T H O D S  ******** */
  ...
  public static String encodeBytes( byte[] source, int off, int len )
  {
    ...
    byte[] outBuff = new byte[   ( len43 )         // Main 4:3
                   ...
                   + ( len43 / MAX_LINE_LENGTH )   // Carriage Returns
    ...
    for( ; d < len2; d+=3, e+=4 )
    {
      ...
      {
        //don't want to output crlf if on last line
        if (source.length != ( d + 3 ) ){
          outBuff[e+4] = CARRIAGE_RETURN;
          outBuff[e+5] = NEW_LINE;
          e+=2;
          lineLength = 0;
        }
      }   // end if: end of line
  ...
  // encode InputStream and return String
  public static String encode(java.io.InputStream is)
  {
  ...
  {
    return new Base64.OutputStream(os);
  }

/* ********  D E C O D I N G   M E T H O D S  ******** */ 
/**
 * The following returns a recoded string based on a string base64 representation
 * @param base64 string representation
 * @return String recoded
 */ 
  public static String recodeString(String s)
  {
     return encodeBytes(decode(s));
  }
 ...
 */
  ...
  public static java.io.InputStream decode(java.io.InputStream is)
  {
    ...
    //decode byte array
    byte[] decodedBytes = decode(out.toByteArray());

    //transfer decoded byte array back to InputStram
    return new ByteArrayInputStream(decodedBytes);
  }

  
  // Wrap a decoder around the given input stream.  InputStream will be decoded as read is called.
  public static java.io.InputStream decodeInputStream(java.io.InputStream is)
  {
    return new Base64.InputStream(is);
  }
    ...
    public void flush() throws java.io.IOException
    {
      if( position > 0 )
      {
        if( encode )
        {
          ...
          position = 0;  //avoid exessive flushing since close() calls flush()
        }   // end if: encoding
        else
        {
