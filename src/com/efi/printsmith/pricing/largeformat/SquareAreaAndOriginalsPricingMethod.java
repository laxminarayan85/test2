	...
	public Job priceLargeFormatJob (Job job) throws Exception {
		...
		if (pricingMatrix != null) {
			...
			for (i = 0; i < pricingMatrix.getElements().size(); i++) {
				...
				if (pricingMatrix.getElements().get(i).getLastLine() == false && pricingMatrix.getElements().get(i).getQty() >= area) break;
			}
			...
			if (copierDefinition.getMatrixType().equals("CopyCost")) {
				...
				if (job.getDoubleSided()) {
					...
					if (side2PricingMethod.equals(Price2Side.UsingSideFactor.name())) {
						pricePerCopy = (pricePerCopy * copierDefinition.getSideTwoFactor()) / 2;
						stockTotalPrice = (stockPrice*((area/2)/runout));
						laborTotalPrice = (pricePerCopy * (area));
					} else if (copierDefinition.getPriceTwoSide().equals(Price2Side.NotChangingPrice.name())) {
						stockTotalPrice = (stockPrice*((area/2)/runout));
						laborTotalPrice = (pricePerCopy * (area / 2));
					} else{
						stockTotalPrice = (stockPrice*((area/2)/runout));
